...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try{
            ...
            Manage manage = JSON.parseObject(userInfo,Manage.class);
            System.out.println("Redis获取到:"+manage.getEmail());
            if(Objects.isNull(manage)){
                throw new RuntimeException("未查询到此用户");
            }
            //TODO 获取权限信息到authenticationToken
            UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(manage,null,null);
